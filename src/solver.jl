...
function sc_weight_covariates(
  Y::Matrix{Float64}, X::Vector; zeta_lambda = 0, zeta_omega = 0, 
  ...
  if isnothing(beta) beta = zeros(size(X)) end
  ...
  while (t < max_iter) && ((t < 2) || (vals[t - 1] - vals[t] > min_decrease ^ 2))
    ...
    t += 1
    grad_beta = (weights["err_lambda"]' .* [arr[1:N0, :] for arr in X] .* vcat(weights["lambda"], -1))[1] / N0 + 
    (weights["err_omega"]' .* [arr[:, 1:T0] for arr in X]' .* vcat(weights["omega"], -1))[1] / T0
